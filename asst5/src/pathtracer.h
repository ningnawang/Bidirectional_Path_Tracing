...
class PathTracer {
 ...
 private:
  ...
  void generate_path(Ray &r, std::vector<StaticScene::VertexInfo> &vertices, bool isEye);

  
  Spectrum evaluatePath(std::vector<StaticScene::VertexInfo> &eyePath, 
                        std::vector<StaticScene::VertexInfo> &lightPath, 
                        int nEye, int nLight);


  Spectrum renderPaths(Ray &eyeRay, Ray &ligthRay, Spectrum &Le, bool includeLe);
